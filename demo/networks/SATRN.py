...
class LocalityAwareFeedforward(nn.Module):
    def __init__(self, filter_size=600, hidden_dim=300, dropout=0.1):
        super(LocalityAwareFeedforward, self).__init__()
        self.layers = nn.Sequential(
            nn.Conv2d(hidden_dim,filter_size,kernel_size=3,padding=1),
            nn.ReLU(True),
            nn.Conv2d(filter_size,hidden_dim,kernel_size=3,padding=1),
            nn.ReLU(True)
        )
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, input,h,w):
        ...
        tmp = tmp.view(tmp.shape[0], tmp.shape[1], h, w)
        ff = self.feedforward_layer(tmp)
        ff = ff.view(ff.shape[0], ff.shape[1], h * w).transpose(1, 2)
